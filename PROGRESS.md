...
- 2026-07-17 — **Auditoria proativa adicional** (varredura de H1 único por página — 18/18 ok; cobertura de `alt` em todas as imagens — ok). **Risco de compliance resolvido, finalmente**: os depoimentos da Home (nomes/empresas com cara de fabricados: "João Silva — CEO na TechSolutions" etc.) haviam sido flagueados numa auditoria anterior (CDC art. 37, CONAR Anexo Q) mas nunca corrigidos. Convertido de "depoimento de pessoa nomeada" para "resultado por segmento" (sem nome/empresa específica) — muda a natureza legal do conteúdo (deixa de impersonar identidade fabricada), mas **os números em si (CPA R$150→R$45, +280% conversão) ainda não têm confirmação do cliente sobre serem reais** — substituir por resultados reais (EMOPS já liberado) quando possível.
- 2026-07-17 — **Breadcrumbs visíveis implementados** em todas as 13 subpáginas que já tinham o schema.org `BreadcrumbList` (invisível) mas nenhuma trilha visual. Criado `Breadcrumbs.astro` (nav semântico, `aria-current` na página atual, `flex-wrap` para não estourar em telas estreitas). Cada página extrai os items numa const própria, reaproveitada tanto pro JSON-LD quanto pro componente visual — uma única fonte de verdade. Testado: build sem erros, breadcrumb de 3 níveis correto, sem overflow em 320px mesmo no rótulo mais longo ("Gestão de Anúncios Online").
- 2026-07-17 — **Auditoria mobile-first/responsivo** (cliente perguntou "o site é mobile-first ou no mínimo responsivo?"). Verificado, não só afirmado: (1) zero classes `max-md:`/`max-lg:` no código — confirma que o CSS foi escrito mobile-first de verdade (Tailwind: classe sem prefixo = base/mobile, `md:`/`lg:` escalam pra cima), não é responsivo "por engenharia reversa" de um design desktop-first; (2) testado ao vivo em 320px, 375px, 414px, 768px e 1280px em Home, `/calculadora`, `/growth` e `/servicos/gestao-anuncios-online` — zero overflow horizontal em qualquer combinação; (3) viewport meta tag correta, tipografia com `clamp()` fluido. **Achado real durante a auditoria**: botão do menu hambúrguer estava em 36x36px, abaixo do mínimo recomendado de 44x44 (WCAG 2.5.5 / iOS HIG) para alvo de toque — corrigido para 44x44, reverificado sem overflow em 320px.
- 2026-07-17 — **Categoria "Ferramentas" adicionada ao menu principal** (cliente notou que faltava): antes as 3 ferramentas gratuitas (calculadora, simulador de site, simulador de funil) só eram alcançáveis via `/links` ou dentro do conteúdo de cada página de serviço, sem rota direta no menu. Aproveitei para refatorar o Nav.astro e eliminar duplicação entre desktop/mobile (os grupos de link agora vivem num único array, mapeado nos dois lugares — evita o tipo de drift que causou esse gap) e adicionar **estado ativo** (`aria-current="page"` + destaque visual) no item/dropdown da página atual, algo que não existia antes. **Melhoria identificada mas não implementada ainda**: breadcrumbs visíveis nas subpáginas (já temos o schema.org `BreadcrumbList` estruturado, só falta o componente visual correspondente) — vale considerar quando mexermos de novo na navegação.
